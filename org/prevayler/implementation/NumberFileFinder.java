...
/** Finds the last .snapshot file by number and finds all the subsequent 
 * pending .log files.
 */
class NumberFileFinder {
	...
	public NumberFileFinder(String directoryName) throws IOException {
		...
		if (!directory.isDirectory())
			throw new IllegalArgumentException(
				"Path exists but is not a directory: " + directoryName);
	...
	*/
	...
	private void findLastSnapshot() throws IOException {
		...
		if (snapshots == null)
			throw new IOException(
				"Error reading file list from directory " + directory);
	...
	private void init() throws IOException {
		findLastSnapshot();
		fileNumber = lastSnapshot == null ? 0 : number(lastSnapshot);
	}
	...
	*/
	public File nextPendingLog() throws EOFException {
		File log =
			new File(
				directory,
				NumberFileCreator.LOG_FORMAT.format(fileNumber + 1));
		if (!log.exists()) {
